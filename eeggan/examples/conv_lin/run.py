...
#
# modelpath = '/data/schirrmr/hartmank/data/GAN/models/GAN_debug/%s/' % (
#             'PAPERFIN4_' + subj_names[subj_ind] + '_FFC4h_WGAN_adaptlambclamp_CONV_LIN_10l_run%d' % task_ind)
	...
	for i_block in range(i_block_tmp, n_blocks):
		...
		for i_epoch in range(i_epoch_tmp, block_epochs[i_block]):
			...
			# for it in range(iters):
				...
				# eegs = eegs.cuda()
				eegs = discriminator.model.downsample_to_block(Variable(eegs[:, :, :, None].view(64, 1, input_length, 1), requires_grad=False), discriminator.model.cur_block).cuda()
